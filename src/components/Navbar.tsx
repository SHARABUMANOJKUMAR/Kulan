    ...
    <header className="fixed w-full z-50 top-0 transition-all duration-300">
      ...
      <nav className={`w-full transition-all duration-300 ${scrolled ? 'bg-primary-bg/95 backdrop-blur-md border-b border-white/10 py-4 shadow-lg' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-8 flex items-center justify-between">
          {/* Logo */}
          <Link to="/" className="font-heading font-bold text-2xl tracking-tight flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-gold to-yellow-600 flex items-center justify-center text-primary-bg font-bold">K</div>
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="px-6 py-4 flex flex-col gap-4">
                ...
                <button className="w-full mt-4 px-6 py-3 rounded-full bg-gold text-primary-bg font-semibold text-sm">
                  Get Started Now
                </button>
              </div>
            </motion.div>
